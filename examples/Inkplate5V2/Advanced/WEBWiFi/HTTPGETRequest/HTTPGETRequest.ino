 ...
 * @brief       Example showing how to make an HTTP GET request and display the response on Inkplate 5V2
 *
...
// Create Inkplate instance in 1-bit mode (black & white mode, no grayscale)
...
void setup() {
  ...
  inkplate.display();   // Perform a full screen refresh to show initial message
  ...
  Serial.println("\nConnected to WiFi!");
  lv_label_set_text(label, "Connected to WiFi!");  // Update message on screen
  lv_timer_handler();
  inkplate.partialUpdate();  // Use partial update for quick refresh
  delay(1000);               // Wait briefly before proceeding

  // Indicate start of HTTP GET request
  lv_label_set_text(label, "Requesting example.com...");
  lv_timer_handler();
  ...
  if (httpCode > 0) {
    ...
    // Truncate the response if it’s too long for the screen
    ...
    lv_obj_set_width(responseLabel, 1000);                          // Limit text width to screen
    lv_obj_set_style_text_color(responseLabel, lv_color_black(), 0);
    lv_obj_set_style_text_font(responseLabel, &lv_font_montserrat_14, 0); // Smaller font for long text
