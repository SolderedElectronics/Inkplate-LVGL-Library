 ...
 *              and display the response on Inkplate 2 using LVGL.
 *
...
void setup() {
  ...
  if (httpCode > 0) { // If HTTP response was received
    ...
    lv_obj_set_width(respLabel, 212);             // Limit text width to prevent overflow
    lv_obj_set_style_text_color(respLabel, lv_color_black(), 0);
    lv_obj_set_style_text_font(respLabel, &lv_font_montserrat_8, 0); // Smaller font for response
