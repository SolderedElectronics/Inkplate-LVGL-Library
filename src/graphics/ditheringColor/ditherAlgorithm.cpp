...
// ------------------
RGBTRIPLE DitherAlgorithm::map_pixel_classic(int _r, int _g, int _b, uint16_t* palette, uint8_t* palette_indices, uint8_t palette_size)
{
    ...
    for (uint8_t j = 0; j < palette_size; j++)
    {
        ...
        uint64_t dist = ((uint64_t)(dr*dr)*30)
                      + ((uint64_t)(dg*dg)*59)
                      + ((uint64_t)(db*db)*11);
...
RGBTRIPLE DitherAlgorithm::map_pixel_fast(int _r, int _g, int _b)
{
    ...
    for (uint8_t j = 0; j < palette_size; j++)
    {
        ...
        if (dist < min_dist) {
            ...
            int bestY = 2 * ((best_color >> 11) & 0x1F)
                      + 5 * ((best_color >> 5) & 0x3F)
                      + (best_color & 0x1F);
            ...
            if (brightnessErr < bestBrightnessErr) {
                ...
                int bestSat = abs(bestPr - bestPg)
                            + abs(bestPg - bestPb)
                            + abs(bestPb - bestPr);

                // If candidate is at least as saturated as best, prefer it.
                if (abs(srcSat - palSat) < abs(srcSat - bestSat) ||
                    palSat > bestSat) {
                    better = true;
                }
            }
...
RGBTRIPLE DitherAlgorithm::map_pixel_HSV(int _r, int _g, int _b, uint16_t* palette, uint8_t* palette_indices, uint8_t palette_size)
{
    float ph, ps, pv;
...
void DitherAlgorithm::ditherFramebuffer(uint8_t* frameBuffer, int width, int height) {
    ...
    for (int y = 0; y < height; y++) {
        ...
        if (!pixels[y]) {
            for (int i = 0; i < y; i++) free(pixels[i]);
            free(pixels);
            return;
        }
    }
